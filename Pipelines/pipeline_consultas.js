[
    {
        $set: {
            ...
            // Gerar população alvo aleatória (70% Adulto, 30% Criança)
            TargetPopulation: { 
                $ifNull: [
                    "$TargetPopulation", 
                    { $cond: [{ $lt: [{ $rand: {} }, 0.7] }, "Adulto", "Criança"] }
                ] 
            },
            ...
            // Gerar tempos de espera aleatórios baseados em valores realistas
            "AverageWaitingTime.Normal": { 
                $toDouble: { 
                    $ifNull: [
                        "$AverageWaitingTime_Normal", 
                        { $add: [60, { $multiply: [{ $rand: {} }, 180] }] }  // 60-240 dias
                    ] 
                } 
            },
            "AverageWaitingTime.Prioritario": { 
                $toDouble: { 
                    $ifNull: [
                        "$AverageWaitingTime_Prioritario", 
                        { $add: [20, { $multiply: [{ $rand: {} }, 60] }] }  // 20-80 dias
                    ] 
                } 
            },
            "AverageWaitingTime.MuitoPrioritario": { 
                $toDouble: { 
                    $ifNull: [
                        "$AverageWaitingTime_MuitoPrioritario", 
                        { $add: [5, { $multiply: [{ $rand: {} }, 25] }] }  // 5-30 dias
                    ] 
                } 
            },
